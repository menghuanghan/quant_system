...
class GRUTimeSeriesSplitter:
    ...
    - Expanding: Train 起点锚定全局最小日期，仅终点向未来延伸
    - Single_Full: 只产出一个 Fold，Train 涵盖全部历史，仅截取最后 1 个月做早停
    ...
    def _split_rolling(self, tw: int, vw: int, step: int) -> Generator[GRUFoldInfo, None, None]:
        ...
        """
        fold_idx = 0
        min_date = pd.Timestamp(self.trade_dates[0])
        max_date = pd.Timestamp(self.trade_dates[-1])
        train_start = min_date
        min_valid_days = int(vw * 30 * 0.5)

        while True:
            train_end = train_start + relativedelta(months=tw) - timedelta(days=1)
            if pd.Timestamp(train_end) > max_date:
                break
            ...
            if valid_start > max_date:
                break
            valid_end = min(pd.Timestamp(valid_end), max_date)
    ...
    def _split_single_full(self) -> Generator[GRUFoldInfo, None, None]:
        ...
        valid_end = max_date
        valid_start = max_date - relativedelta(months=1)
        ...
        if len(train_end_candidates) <= self.gap_days:
            raise ValueError("Not enough data for single_full split")
        train_end = train_end_candidates[-(self.gap_days + 1)]

        train_indices = self._get_warmup_indices(min_date, pd.Timestamp(train_end))
        valid_indices = self._get_warmup_indices(valid_start, valid_end)
        ...
        logger.info(
            ...
            f"train=[{min_date.date()}, {pd.Timestamp(train_end).date()}] "
            f"({len(train_indices)} rows), "
            f"valid=[{valid_start.date()}, {valid_end.date()}] "
            f"({len(valid_indices)} rows)"
        )
